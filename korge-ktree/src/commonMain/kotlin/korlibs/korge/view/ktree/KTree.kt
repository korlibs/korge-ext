...
package korlibs.korge.view.ktree
...
import korlibs.korge.view.grid.*
...
@Deprecated("KTree is going to be removed in a future version") val Views.ktreeSerializerHolder get() = extraCache("ktreeSerializerHolder") { object :
    KTreeSerializerHolder { override val serializer: KTreeSerializer get() = ktreeSerializer } }
...
open class KTreeSerializer(val views: Views) : KTreeSerializerHolder, Extra by Extra.Mixin() {
    ...
    open suspend fun ktreeToViewTreeInternal(xml: Xml, currentVfs: VfsFile, parent: Container?): View {
        ...
        when (xml.nameLC) {
            ...
            "ellipse" -> view = Ellipse(Size(50f, 50f), Colors.RED)
            ...
            "ninepatch" -> view = NinePatch(NinePatchBitmap32(Bitmap32(62, 62, premultiplied = true)))
        ...
        fun float(prop: KMutableProperty0<Float>, defaultValue: Float) {
            prop.set(xml.float(prop.name, defaultValue))
        }
        ...
        if (view is Anchorable) {
            view::anchor.set(Anchor(xml.float("anchorX", 0f), xml.float("anchorY", 0f)))
        }
        if (view is RectBase) {
            view::size.set(Size(xml.float("width", 100f), xml.float("height", 100f)))
        }
        if (view is NinePatch) {
            view::size.set(Size(xml.float("width", 100f), xml.float("height", 100f)))
        }
        if (view is Text) {
    ...
    open fun viewTreeToKTree(view: View, currentVfs: VfsFile, level: Int): Xml {
        ...
        if (view is RectBase) {
            if (view.anchor != Anchor.TOP_LEFT) {
                properties["anchorX"] = view.anchor.sx
                properties["anchorY"] = view.anchor.sy
            }
            add(view::width)
            add(view::height)
        }
        ...
        var out = if (registration != null) {
            ...
            result ?: when (view) {
                is NinePatch -> Xml("ninepatch", rproperties)
...
@Deprecated("KTree is going to be removed in a future version")
class KTreeRoot(width: Double, height: Double) : FixedSizeContainer(Size(width, height)) {
    ...
    override fun renderInternal(ctx: RenderContext) {
        ...
        if (showGrid) {
            grid.draw(ctx, width.toDouble(), height.toDouble(), globalMatrix)
        }
    }
}
